...
function useMetaMaskInternal(): UseMetaMaskState {
  ...
  const saveWalletState = useCallback(
    (connectorId: string | null, accounts: string[] | null, chainId: number | null, connected: boolean) => {
      if (typeof window === "undefined") return;
      try {
        if (connectorId !== null) {
          localStorage.setItem(WALLET_STORAGE_KEY.CONNECTOR_ID, connectorId);
        }
        if (accounts !== null) {
          localStorage.setItem(WALLET_STORAGE_KEY.ACCOUNTS, JSON.stringify(accounts));
        }
        if (chainId !== null) {
          localStorage.setItem(WALLET_STORAGE_KEY.CHAIN_ID, String(chainId));
        }
        localStorage.setItem(WALLET_STORAGE_KEY.CONNECTED, String(connected));
      } catch (e) {
  ...
  const loadWalletState = useCallback(() => {
    ...
    try {
      const connected = localStorage.getItem(WALLET_STORAGE_KEY.CONNECTED) === "true";
      const connectorId = localStorage.getItem(WALLET_STORAGE_KEY.CONNECTOR_ID);
      const accountsStr = localStorage.getItem(WALLET_STORAGE_KEY.ACCOUNTS);
      const chainIdStr = localStorage.getItem(WALLET_STORAGE_KEY.CHAIN_ID);
      
      if (!connected || !connectorId) return null;
      
      return {
        connectorId,
        accounts: accountsStr ? JSON.parse(accountsStr) : null,
  ...
  const clearWalletState = useCallback(() => {
    if (typeof window === "undefined") return;
    try {
      localStorage.removeItem(WALLET_STORAGE_KEY.CONNECTOR_ID);
      localStorage.removeItem(WALLET_STORAGE_KEY.ACCOUNTS);
      localStorage.removeItem(WALLET_STORAGE_KEY.CHAIN_ID);
      localStorage.removeItem(WALLET_STORAGE_KEY.CONNECTED);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (next) {
      nextConnectListener = (connectInfo: ProviderConnectInfo) => {
        ...
        const newChainId = Number.parseInt(connectInfo.chainId, 16);
        _setCurrentProvider(next);
        _setChainId(newChainId);
        saveWalletState("metamask", null, newChainId, true);
      };
      ...
      nextAccountsChangedListener = (accounts: string[]) => {
        ...
        if (accounts.length > 0) {
          saveWalletState("metamask", accounts, null, true);
        } else {
          clearWalletState();
        }
      };
      ...
      const updateChainId = async () => {
        ...
        try {
          ...
          const shouldSilentReconnect = savedState?.connected && savedState.connectorId === "metamask";

          if (shouldSilentReconnect) {
            const accountsArray = await next.request({ method: "eth_accounts" });
            if (accountsArray && accountsArray.length > 0) {
              const chainIdHex = await next.request({ method: "eth_chainId" });
              const newChainId = Number.parseInt(chainIdHex as string, 16);
              console.log(
                `[useMetaMask] silent reconnect chainId=${chainIdHex} accounts.length=${accountsArray.length}`
              );
              _setCurrentProvider(next);
              _setChainId(newChainId);
              _setAccounts(accountsArray);
              saveWalletState("metamask", accountsArray, newChainId, true);
              return;
            }
          }
          ...
          if (accountsArray && accountsArray.length > 0) {
            saveWalletState("metamask", accountsArray, Number.parseInt(chainIdHex as string, 16), true);
          }
        } catch {
  ...
  }, [providers, saveWalletState, loadWalletState, clearWalletState]);
